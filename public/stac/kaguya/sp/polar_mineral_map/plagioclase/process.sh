#!/usr/bin/env bash
# Generate COG GeoTIFFs from Kaguya SP gridded plagioclase mosaics, north and south polar regions (Lemelin et al. 2022)
#
# Source files (Zenodo record 7108163, version 2, CC-BY-4.0), read in place from the zips:
#   gridded_plagioclase_mosaic_north_pole.tiff.zip
#   gridded_plagioclase_mosaic_south_pole.tiff.zip
...
#
# Output : plagioclase_kaguya_sp_lemelin2022_north_pole_COG.tif (wt%, IAU:30130, north-up)
#          plagioclase_kaguya_sp_lemelin2022_south_pole_COG.tif (wt%, IAU:30135, north-up)
# GDAL   : 3.12.2
#
...
process_pole() {
  local pole="$1"
  local crs="$2"
  local name="gridded_plagioclase_mosaic_${pole}"
  local out="plagioclase_kaguya_sp_lemelin2022_${pole}_COG.tif"
  local vrt="plagioclase_${pole}_georef_tmp.vrt"
  local tmp="plagioclase_${pole}_wtpct_tmp.tif"
