#!/usr/bin/env bash
# Generate COG GeoTIFFs from Kaguya SP gridded olivine mosaics, north and south polar regions (Lemelin et al. 2022)
#
# Source files (Zenodo record 7108163, version 2, CC-BY-4.0), read in place from the zips:
#   gridded_olivine_mosaic_north_pole.tiff.zip
#   gridded_olivine_mosaic_south_pole.tiff.zip
...
#
# Output : olivine_kaguya_sp_lemelin2022_north_pole_COG.tif (wt%, IAU:30130, north-up)
#          olivine_kaguya_sp_lemelin2022_south_pole_COG.tif (wt%, IAU:30135, north-up)
# GDAL   : 3.12.2
#
...
process_pole() {
  local pole="$1"
  local crs="$2"
  local name="gridded_olivine_mosaic_${pole}"
  local out="olivine_kaguya_sp_lemelin2022_${pole}_COG.tif"
  local vrt="olivine_${pole}_georef_tmp.vrt"
  local tmp="olivine_${pole}_wtpct_tmp.tif"
